...
public class QueueTableDaoImpl implements QueueTableDao {
    ...
    public int save(final QueueTable queueTable) {
        String sql = "insert into queue_table_customer(request,type,status,entry_date,comment)"
                     + "values(?,?,?,?,?)";
    ...
    public List<QueueTable> queryAll() {
        String sql = "select * from queue_table_customer";
    ...
    public Integer updateStatus(final QueueTable queueTable) {
        String sql = "update queue_table_customer set status=? where id =?";
        return jdbcTemplate.update(sql, new PreparedStatementSetter() {
    ...
    public void clearMessage() {
        String sql = "delete from queue_table_customer";
        jdbcTemplate.update(sql);
    }

    @Override
    public QueueTable quryByEntryDate(Date entryDate) {
        String sql = "select * from queue_table_customer where  timestampdiff(microsecond,'" + entryDate + "',entry_date)  = 0";
    ...
    public void updateComment(final QueueTable queue) {
        String sql = "update queue_table_customer set comment=? where id =?";
